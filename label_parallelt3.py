...
import math
import sys
#Attempt to parallelize the output part (writing table 3 on file)
#Usage: python label_parallelt3.py nproc cftable
...
#Slice of the function that writes the output (each process writes to a different file, files are supposed to be merged at the end)
def writeslice(g, beg, end, cfed):
    for i in cfed.keys()[beg, end]:
        shortest = g.shortest_paths(source = g.vs.find(cfed[i]).index)
        os.system('echo '+str(shortest[0])+' >> table'+str(beg)+'_3')
...
jobs = [job_server.submit(writeslice,(g,i*n/m,(i+1)*n/m,cfed),(),('os',)) for i in range(m)]
