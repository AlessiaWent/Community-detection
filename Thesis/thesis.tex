...
The whole activity was co-supervised by Generali Big data group (dr. Valerio Consorti), which suggested for the greatest part the way to proceed. We remark that some deliverables have been submitted to Genertel and intermediate results were periodically discussed during several meetings.
...
\item average degree
\item maximum length of a shortest path between a pair of objects in the cluster
...
\subsection{Dealing with graphs} 
  igraph is an open source graph package, distributed under the terms of the GNU GPL.
